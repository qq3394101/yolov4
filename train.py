...
# net : model = YoloBody(len(anchors[0]), num_classes)
# yolo_loss
# Epoch 50 冻结
    ...
    with tqdm(total=epoch_size,desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen):
            if iteration >= epoch_size:
                break
            images, targets = batch[0], batch[1] # N 3 416 416 N n 5
            with torch.no_grad():
                if cuda:
                    images  = torch.from_numpy(images).type(torch.FloatTensor).cuda()
                    targets = [torch.from_numpy(ann).type(torch.FloatTensor) for ann in targets] # [tensor0,tensor1]
                else:
...
if __name__ == "__main__":
    ...
    # root.jpg 1,1,2,2,0 1,1,2,2,0 1,1,2,2,0
    #----------------------------------------------------------------------#
    ...
    if True:
        ...
        for param in model.backbone.parameters():
            param.requires_grad = False # 不计算backbone梯度
